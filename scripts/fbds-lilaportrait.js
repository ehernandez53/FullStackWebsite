...
(function (window) {
    ...
    class firebasedatastore {
        ...
        async add(key, val) {
            ...
            const docRef = this.db.doc(`lilaportrait/${this.makeDocHash(20)}`);
            return docRef.set(val);
        }

        async get(description, cb) {
            const docRef = this.db.collection(`lilaportrait`);
        ...
        async remove(description) {
            const docRef = await this.db.collection(`lilaportrait`);
